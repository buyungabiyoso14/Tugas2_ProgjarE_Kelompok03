...
def perintah(command,conn):
    ...
    if (check_this_client_online(conn)):
            ...
            if (len(temp) < 3):
                ...
                print 'PESAN PERSONAL TERKIRIM '+nama+' OK\n'
                conn.sendall('301\n')
        elif temp[0]=='LOGOUT':
            conn.sendall('203\n')
            nama = username(conn)
            del online_list[nama]
            print 'BERHASIL LOGOUT! ' + nama + ' LOGOUT\n'
            conn.close()
            ...
            if len(temp)<2:
                ...
                print "GRUP BERHASIL DIBUAT! "+temp[1]+" BY "+nama+" CREATED\n"
                message = "401\n"
                conn.sendall(message.encode('utf-8'))
        elif temp[0]=='JOING':
            nama = username(conn)
            if len(temp) < 2:
                ...
                message = "501\n"
                print "BERHASIL GABUNG GRUP "+nama+" JOIN "+temp[1]+"\n"
                conn.sendall(message.encode('utf-8'))
            ...
            if len(temp) < 3:
                ...
                conn.sendall('701\n')
                print "PESAN GRUP TERKIRIM "+temp[1]+" "+nama+" OK\n"
        elif temp[0] == 'KELUAR':
            if len(temp) < 2:
                message = "001\n"
                conn.sendall(message.encode('utf-8'))
                ...
                conn.sendall('601\n')
                print 'BERHASIL KELUAR GRUP '+nama+' LEAVE '+temp[1]+'\n'
        elif temp[0]=='SIGNUP' or temp[0]=='MASUK':
            message = "003\n"
            conn.sendall(message.encode('utf-8'))
        else:
            message="001\n"
        ...
        if len(temp) < 3:
            ...
            if regist == 103:
                ...
                message = "101\n"
                print "BERHASIL SIGNUP "+temp[1]+" OK\n"
                conn.sendall(message.encode('utf-8'))
    elif temp[0] == 'MASUK':
        if len(temp) < 3:
            message = "001\n"
            ...
            if loginn == 201:
                uName = temp[1]
                message = "201\n"
                print "BERHASIL LOGIN " + temp[1] + " OK\n"
                conn.sendall(message.encode('utf-8'))
